...
function handleRequest(req, res) {
  ...
  return isAvailableOnS3(ctx, function(err, isAvailable) {
    if (err) {
      ...
      return sendError(res, 500, 'isAvailableOnS3 error');
    }
...
function getS3Url(region, backend, url) {
  var regionOptions = config.regions[region];
  var backendOptions = config.backends[backend];
  var bucket = getBucketForRegion(region);
  var path = getS3Path(backend, url);
  var url = 'https://' + regionOptions.endpoint + '/' + bucket + path;
  return url;
}
...
function redirectToS3(res, ctx) {
  var s3Url = getS3Url(ctx.region, ctx.backend, ctx.url);
  res.writeHead(302, {
    'Location': s3Url
  });
  res.end();
  console.log('response: HTTP 302:', s3Url);
}
